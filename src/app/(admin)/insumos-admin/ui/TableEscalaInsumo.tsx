...
import { CrearEscalaInsumo } from "@/apis/escalas-insumos/accions/crear-escala-insumo";
import { ActualizarEscalaInsumo } from "@/apis/escalas-insumos/accions/actualizar-escala-insumo";
...
import { Insumo } from "@/apis/descuentos-insumos/interfaces/response-descuentos-insumo.interface";
import { CrearEscalaInsumoInterface } from "@/apis/escalas-insumos/interfaces/crear-escala-insumo.interface";
import { Escala } from "@/apis/escalas-insumos/interfaces/response-escala-insumo.interface";
import useGetEscalasInsumos from "@/hooks/escalas-insumos/useGetEscalasInsumos";
...
const TableEscalaInsumo = ({ selectedInsumo, user }: Props) => {
  ...
  const { data: escalasData, isLoading } = useGetEscalasInsumos(
    limit,
    offset,
    selectedInsumo?.id ?? ""
  );
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<CrearEscalaInsumoInterface>();

  const [escalas, setEscalas] = useState<EscalaLocal[]>([]);

  const createMutation = useMutation({
    mutationFn: CrearEscalaInsumo,
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["escalas-insumo", limit, offset, selectedInsumo?.id],
      });
      queryClient.invalidateQueries({
        queryKey: ["escalas-proveedor-insumo"],
      });
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: {
      ...
      return ActualizarEscalaInsumo(data.id, updateData);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["escalas-insumo", limit, offset, selectedInsumo?.id],
  ...
  const onSubmit = (data: CrearEscalaInsumoInterface) => {
    if (!selectedInsumo) {
      ...
      <div className="bg-muted p-4 rounded-lg">
        ...
        >
          ...
          <Button
            type="submit"
            className="w-full"
            disabled={createMutation.isPending || !selectedInsumo}
          >
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <br />
              <br />
              <strong>Insumo:</strong> {selectedEscala?.insumo.nombre}
              <br />
...
export default TableEscalaInsumo;
